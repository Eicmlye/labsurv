...
class SumTree:
    ...
    def refresh(self, qnet: Module, target_qnet: Module, gamma: float):
        """
        ...
        for data_index in range(1, 1 + len(self)):
            ...
            with torch.no_grad():
                ...
                values_predict: Tensor = torch.gather(
                    qnet(cur_observation),
                    dim=1,
                    index=cur_action,
                )
                td_target = reward + gamma * torch.max(
                    target_qnet(next_observation),
                    dim=1,
                )[0].view(-1, 1) * (1 - terminated)

                td_error = td_target - values_predict
...
class CliffWalkPrioritizedReplayBuffer(BaseReplayBuffer):
    """
    ## Description:
    ...
    def update(self, qnet: Module, target_qnet: Module, gamma):
        """
        Update the priority of all the nodes.
        """
        self._buffer.refresh(qnet, target_qnet, gamma)
