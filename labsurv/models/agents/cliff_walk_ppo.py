...
class CliffWalkPPO(BaseAgent):
    ...
    def update(self, transitions: dict, logger: LoggerHook):
        ...
        cache_log_probs: Tensor = torch.log(
            torch.gather(
        ...
        for step in range(self.update_step):
            cur_log_probs: Tensor = torch.log(
                torch.gather(
                    self.actor(cur_observations),
                    dim=1,
                    index=cur_actions,
            ...
            ratio = torch.exp(cur_log_probs - cache_log_probs)
            ...
            actor_loss: Tensor = torch.mean(-torch.min(surrogate_1, surrogate_2))
