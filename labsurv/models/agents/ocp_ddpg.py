...
class OCPDDPG(BaseAgent):
    ...
    def train_take_action(self, observation: array) -> array:
        ...
        with torch.no_grad():  # if grad, memory leaks
            action_with_params: Tensor = self.actor(observation.unsqueeze(0))  # B * 7
    ...
    def update(
        ...
        critic_loss = torch.mean(
            F.mse_loss(
                self.critic(cur_observations, cur_action_with_params).squeeze(1),  # [B]
                discounted_target_q,
            )
        )
        ...
        actor_loss = -torch.mean(
            self.critic(cur_observations, self.actor(cur_observations))
        )
