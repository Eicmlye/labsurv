            ...
            for index in range(self.env.shape[0]):
                for jndex in range(self.env.shape[1]):
                    ...
                    for action in range(len(self.env.actions)):
                        prob, next_state, reward, done = self.env.transition[cur_state][
                            action
                        ]
                        if done is not None:
                            new_state_value[cur_state] += self.strategy[cur_state][
                                action
                            ] * (
                                reward
                                + self.gamma * prob * self.state_value[next_state]
                            )

                    max_diff = max(
                        max_diff,
                        abs(new_state_value[cur_state] - self.state_value[cur_state]),
                    )
        ...
        for index in range(self.env.shape[0]):
            for jndex in range(self.env.shape[1]):
                ...
                self.strategy[cur_state] = [
                    (1 / max_count if state_action_value[action] == max_value else 0)
                    for action in range(len(self.env.actions))
                ]
...
def get_init_probs(world, neighbours):
    ...
    for neighbour in neighbours:
        if neighbour[0] in range(world.shape[0]) and neighbour[1] in range(
            world.shape[1]
        ):
