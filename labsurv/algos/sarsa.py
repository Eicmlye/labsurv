...
class SARSAAlgo:
  ...
  def take_action(self, state):
    if np.random.random() < self.epsilon:  # explore
      ...
      out_action_index_among_max = np.random.randint(dict(zip(unique, counts))[max_value])
  ...
  def update(self, cur_state, cur_action, reward, next_state, next_action, terminated):
    ...
    if len(self.memory["state_index"]) == self.step:
      total_reward = self.qtable[next_state_index, next_action]
    ...
    for index in range(self._shape[0]):
      for jndex in range(self._shape[1]):
        ...
        if self._world[index][jndex] == DANGER:
          ...
          strategy_str = ""
          strategy_str += "^" if self.qtable[state_index][0] == max_value else "o"
          strategy_str += "<" if self.qtable[state_index][1] == max_value else "o"
          strategy_str += "v" if self.qtable[state_index][2] == max_value else "o"
          strategy_str += ">" if self.qtable[state_index][3] == max_value else "o"
